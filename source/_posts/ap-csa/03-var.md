...
本章的内容还是在[考纲](../00-about-ap-csa#考试大纲) 1.2 的内容里面，今天讲一个编程中很常见的话题：变量和常量
...
**前一章所讲到的数据类型，正是约束声明这个容器内所存放的内容的手段，整个 Java 都会围绕着类型展开**
...
在整个 Java 的学习当中，类型这个概念永远是贯穿始末的，我在上一章讲述了 Java 有什么类型，这一章讲述了类型的最主要作用是什么——约束了"变量"这一容器的类型

接下来的学习中，我希望你看到代码之后立刻问自己三个问题：
1. 这是什么类型？
2. 这个表达式的结果是什么类型？
3. 这里需要什么类型？

只要能彻底弄明白类型的概念，任何语法都不抽象
