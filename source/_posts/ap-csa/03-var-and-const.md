...
- 分解位数
  拿任意数字 %10，那么我们就可以得到最后一位的数字，比如 `123 % 10` 我们可以得到最后一位 3
...
包括上一章里面的所有数据类型和boolean的值，全部的都列在本章节最后的表里面了，你要脑力充足可以去背，我建议是不用背，变量取名的时候稍微用稍微复杂一点的名字就好了：[Java 保留关键字（Java Reserved Keywords）](#Java 保留关键字（Java Reserved Keywords）)
